...
import { zCreatePost, zUpdatePost } from '~/models/zod'
...
export class PostRouter {
  ...
  create = p.public.input(zCreatePost).mutation(async ({ input }) =>
    e
      .insert(e.Post, {
        ...input,
  ...
  update = p.public.input(zUpdatePost).mutation(async ({ input }) =>
    e
      .update(e.Post, () => ({
        set: input,
      }))
